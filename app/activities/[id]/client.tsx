...
export function AttendeeRequestPanel({
  ...
  if (myStatus === "approved" || myStatus === "confirmed") {
    return (
      <div className="space-y-3">
        <Card border="violet">
          <p className="text-xs text-violet-300 mb-1">✓ You&apos;re in</p>
  ...
  if (myStatus === "pending") {
    return (
      <div className="space-y-3">
        <Card border="violet">
          <p className="text-xs text-violet-300 mb-1">⏳ Request sent</p>
        ...
        <button
          ...
          disabled={busy}
          className="w-full py-3 rounded-xl bg-white/5 border border-white/10 text-white/70 text-sm hover:bg-white/[0.07] disabled:opacity-50"
        >
          Withdraw request
        </button>
        {error && <ErrorBox msg={error} />}
  ...
  if (introOpen) {
    return (
      <div className="space-y-3">
        ...
        />
        <p className="text-xs text-white/30 text-right -mt-2">{introMsg.length}/280</p>
        <div className="grid grid-cols-2 gap-2">
          <button
            onClick={() => setIntroOpen(false)}
            disabled={busy}
            className="py-3 rounded-xl bg-white/5 border border-white/10 text-sm font-medium"
          >
            Cancel
          </button>
          <button
    ...
    <div className="space-y-5">
      ...
      <section>
        <h3 className="font-semibold text-sm mb-2">
          Join requests
          <span className="ml-2 text-white/40 text-xs">{pending.length}</span>
        </h3>
        {pending.length === 0 ? (
          <Card>
            <p className="text-sm text-white/50">No pending requests yet.</p>
          </Card>
        ) : (
      ...
      <section>
        <h3 className="font-semibold text-sm mb-2">
          Approved
          <span className="ml-2 text-white/40 text-xs">{approved.length}</span>
        </h3>
        {approved.length === 0 ? (
          <Card>
            <p className="text-sm text-white/50">No approved attendees yet.</p>
          </Card>
        ) : (
          <div className="space-y-2">
            {approved.map((a) => (
    ...
    <Card>
      <Link href={`/profile/${u.id}`} className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {studyOrJob && (
            <p className="text-xs text-white/50 truncate">{studyOrJob}</p>
          )}
      ...
      {attendee.requested_message && (
        <div className="mt-3 p-3 bg-black/30 rounded-xl border border-white/5">
          <p className="text-[11px] text-white/40 mb-1">Their note</p>
      ...
      <div className="grid grid-cols-2 gap-2 mt-3">
        <button
          ...
          disabled={busy !== null}
          className="py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm font-medium hover:bg-white/[0.07] disabled:opacity-50"
        >
          {busy === "reject" ? "…" : "Decline"}
        </button>
        <button
  ...
  return (
    <Card>
      ...
      <div className="grid grid-cols-2 gap-2 mt-3">
        ...
        {revealed ? (
          <span className="py-2.5 rounded-xl bg-violet-500/10 border border-violet-500/20 text-sm font-medium text-violet-300 text-center">
            ✓ Revealed
          </span>
        ) : (
          <button
