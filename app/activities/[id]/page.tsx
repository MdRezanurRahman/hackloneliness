...
export default async function ActivityDetailPage({
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-black dark:bg-black text-slate-900 dark:text-white pb-28">
      <header className="px-5 py-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-900">
        <Link href="/activities" className="text-slate-900/60 dark:text-white/60 text-sm">
          ← Back
        </Link>
      ...
      <div className="max-w-md mx-auto px-5 py-6 space-y-5">
        ...
        <div>
          ...
          <p className="text-slate-900/50 dark:text-white/50 text-sm mt-1 capitalize">{a.category}</p>
        </div>

        {/* ── Host card (always visible — attendees can vet host) ── */}
        <Link
          href={`/profile/${a.host_id}`}
          className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800 transition-colors"
        >
          <Avatar name={a.host?.display_name ?? "Host"} url={a.host?.avatar_url ?? null} size="md" />
          <div className="flex-1 min-w-0">
            <p className="text-xs text-slate-900/50 dark:text-white/50">Hosted by</p>
        ...
        <div className="space-y-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
          ...
          <div className="flex items-center gap-3 pt-1">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-slate-900/40 dark:text-white/40 text-xs">Meeting spot</p>
              {locationRevealedToMe && a.address_label ? (
                <p className="text-sm">{a.address_label}</p>
              ) : (
                <p className="text-sm text-slate-900/40 dark:text-white/40 italic">
        ...
        {a.description && (
          <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
            <p className="text-xs text-slate-900/50 dark:text-white/50 mb-1.5">Details</p>
            <p className="text-sm text-slate-900/90 dark:text-white/90 whitespace-pre-wrap">{a.description}</p>
          </div>
        )}
        ...
        {publicAttendees.length > 0 && (
          <div>
            <p className="text-xs text-slate-900/50 dark:text-white/50 mb-2">Who&apos;s coming</p>
            <div className="flex flex-wrap gap-2">
              {publicAttendees.map((att) => (
                <Link
                  key={att.user_id}
                  href={`/profile/${att.user.id}`}
                  className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-full pl-1 pr-3 py-1"
                >
                  ...
                  {att.user.id === a.host_id && (
                    <span className="text-[10px] text-violet-600 dark:text-violet-400">host</span>
                  )}
                </Link>
              ))}
            </div>
...
function InfoRow({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-slate-900/40 dark:text-white/40 text-xs">{label}</p>
