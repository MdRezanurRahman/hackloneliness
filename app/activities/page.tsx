      ...
      <div className="max-w-3xl mx-auto px-5 py-5 space-y-3">
        {activities.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-slate-900/50 dark:text-white/50 text-sm mb-5">Be the first to host something.</p>
            <Link
              href="/activities/new"
...
function ActivityRow({ activity }: { activity: ActivityWithHost }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800 transition-colors"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-slate-900/50 dark:text-white/50 text-xs">{when} · {activity.city ?? "Nearby"}</p>
        <p className="text-violet-600 dark:text-violet-400/80 text-xs mt-0.5">
