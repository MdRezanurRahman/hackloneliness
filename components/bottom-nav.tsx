...
export function BottomNav() {
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-0 inset-x-0 z-30 bg-white/90 dark:bg-black/90 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 pb-[env(safe-area-inset-bottom)]">
            ...
            return (
              <Link
                ...
                className={`relative flex flex-col items-center justify-center py-1.5 transition-colors ${
                  active ? "text-slate-900 dark:text-white" : "text-slate-900/40 dark:text-white/40"
                }`}
              >
...
function NewMessageToast({
  ...
  return (
    <Link
      ...
      onClick={onClose}
      className="fixed top-3 right-3 left-3 sm:left-auto sm:right-6 sm:top-6 z-50 max-w-sm bg-white/95 dark:bg-black/95 backdrop-blur border border-violet-400/40 rounded-2xl p-3 shadow-2xl shadow-violet-500/30 flex items-center gap-3 animate-toast-in"
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-violet-600 dark:text-violet-400 font-medium">New message</p>
        <p className="text-sm truncate text-slate-900/90 dark:text-white/90">{data.text}</p>
      </div>
